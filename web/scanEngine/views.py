...
from hunter.tasks import (run_command, send_discord_message, send_slack_message, send_telegram_message)
...
def hunter_settings(request, slug):
    context = {}
    ...
    context['settings_nav_active'] = 'active'
    context['hunter_settings_li'] = 'active'
    context['settings_ul_show'] = 'show'

    return render(request, 'scanEngine/settings/hunter.html', context)
...
def notification_settings(request, slug):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            send_slack_message('*hunter*\nCongratulations! your notification services are working.')
            send_telegram_message('*hunter*\nCongratulations! your notification services are working.')
            send_discord_message('**hunter**\nCongratulations! your notification services are working.')
            messages.add_message(
                request,
                messages.INFO,
