function get_external_tool_latest_version(tool_id, tool_name){
  ...
  if (current_version === 'Invalid version lookup command.' || current_version === 'Version Lookup command not provided.'){
    Swal.fire({
      ...
      text: `Since the version lookup command is invalid, hunter is not able to detect if there's a newer version. But you can still force download the latest version.`,
      icon: 'info',
    ...
    .then(function (response) {
      ...
      if (response['message'] == 'RateLimited') {
        ...
        if (current_version === 'Invalid version lookup command.' || current_version === 'Version Lookup command not provided.'){
          Swal.fire({
            title: 'Unable to fetch latest version!',
            text: `Since the version lookup command is invalid, hunter is not able to detect if there's a newer version. But you can still force download the latest version. The latest version is ${latest_version}.`,
            icon: 'info',
            confirmButtonText: 'Update ' +  htmlEncode(tool_name)
          }).then((result) => {
            /* Read more about isConfirmed, isDenied below */
