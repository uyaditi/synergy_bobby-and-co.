...
function detect_subdomain_cms(http_url, http_status){
	if (http_status == 0) {
		var message = `hunter has earlier identified that this subdomain did not return any HTTP status and likely the subdomain is not alive. rhuntermay not be able to detect any CMS, would you still like to continue?`;
	}
	else if (http_status != 200) {
		var message = `hunter has earlier identified that this subdomain has HTTP status as ${http_status} and likely that rhunterwill not detect any CMS, would you still like to continue?`;
	}
